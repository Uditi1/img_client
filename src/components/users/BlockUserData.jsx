...
const BlockUserData = ({ setBlockModal, getAllUsers, setSelectedRowIndex, id, status }) => {
  const onClose = () => {
    setBlockModal(false);
  ...
  return (
    <div className="w-full h-full justify-center items-center flex flex-col gap-4">
      ...
      <div className="flex flex-col ">
        <p className="font-satoshiBold text-[20px] text-center">{status === false ? "Unblock" : "Block"} User</p>
        <p className="font-satoshiRegular leading-[28px] text-[16px] w-[250px] text-center">
          Are you sure you want to {status === false ? "unblock" : "block"} this user?
        </p>
      </div>
      ...
      <div className="flex w-full justify-between">
        ...
        <Button onClick={toggleUser} className="w-[130px] h-[55px] bg-red text-white">Yes</Button>
      </div>
    </div>
  );
};
